    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if ident == "without_views" {
                ...
                return Err(syn::Error::new(
                    ident.span(),
                    "\"with_views\" is no longer needed. Views are generated by default when fallback!() is provided. Use #[routes] with fallback!() in the module body. To disable view generation, use #[routes(without_views)].",
                ));
            } else {
                return Err(syn::Error::new(
                    ident.span(),
